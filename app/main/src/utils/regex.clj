...
(defn named-groups
  ...
  (let [named-groups (->> (re-seq #"\?\<([a-zA-Z0-9]+)\>" (str regex)) (map second))
    ...
    (if (.matches matcher)
      (->> named-groups
           (map (fn [word] {(keyword word) (.group matcher word)}))
           (into {}))
      (throw (ex-info "Can't extract values" {})))
    ))
#_ (named-groups #".*(?<year>\d{4})-(?<month>.{2})-monthly.xml" "/Users/greg/Sources/rynkowski/trading-tax-calc/dev/resources/exrates/gbp/2017-06-monthly.xml")
